...
engine allows generating certificates advantages over classic CA scenarios:
...
Here we create a role `example-dot-com` that allows cetificates with the common name
...
public String issue(String csr) {
    ...
    return pkiSecretEngine.decrypt("my-encryption-key", cipherText).asString();
}
----
...
the `VaultPKISecretEngineFactory` interface.

To enable, or mount, a new PKI engine at specific mount path you can use the `enable` method:

[source, java, subs=attributes+]
----
// Obtain factory via injection or other standard CDI method.
...
pkiSecretEngineFactory.enable("pki-dyn", "A dynamic PKI engine", options);
